...
//Initialize MCU Timer (interrupt every 1ms)
...
int8_t positionCounter = -1; //Index for the servo angle Lookup table
...
int main(void) {
	...
	SysCtlDelay(computeDelayCount(1500, 40)); //Wait 1.5 sec40 MHz on coomputeDelayCount parameter
...
//	//Set servoAngle to 0 degrees
//	setServoAngle(90);
//	SysCtlDelay(computeDelayCount(1500, 40)); //40 MHz on coomputeDelayCount parameter
//	setServoAngle(180);
//	SysCtlDelay(computeDelayCount(1500, 40)); //40 MHz on coomputeDelayCount parameter
//	setServoAngle(0);
